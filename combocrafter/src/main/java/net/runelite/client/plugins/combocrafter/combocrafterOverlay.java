...
@Slf4j
@Singleton
class combocrafterOverlay extends OverlayPanel
{
	private final Client client;
	private final combocrafterPlugin plugin;
	private final combocrafterConfig config;

	String timeFormat;
	private String infoStatus = "Starting...";

	@Inject
	private combocrafterOverlay(final Client client, final combocrafterPlugin plugin, final combocrafterConfig config)
	{
		super(plugin);
		...
		this.plugin = plugin;
		this.config = config;
		getMenuEntries().add(new OverlayMenuEntry(RUNELITE_OVERLAY_CONFIG, OPTION_CONFIGURE, "Combination Crafter overlay"));
	}
